...
class LFWDataset(datasets.ImageFolder):
    def __init__(self, dir, pairs_path, image_size, transform=None, half_face=False):
        super(LFWDataset, self).__init__(dir, transform)
        self.image_size = image_size
        self.pairs_path = pairs_path
        self.validation_images = self.get_lfw_paths(dir, half_face)
    ...
    def get_lfw_paths(self, lfw_dir, half_face, file_ext="jpg"):
        ...
        for i in range(len(pairs)):
            ...
            if len(pair) == 3:
                ...
                if not half_face:
                    ...
                    path1 = os.path.join(lfw_dir, pair[0], pair[0] + '_' + '%04d' % int(pair[2]) + '_h.' + file_ext)
                issame = True
            elif len(pair) == 4:
                path0 = os.path.join(lfw_dir, pair[0], pair[0] + '_' + '%04d' % int(pair[1]) + '.' + file_ext)
                if not half_face:
                    ...
                    path1 = os.path.join(lfw_dir, pair[2], pair[2] + '_' + '%04d' % int(pair[3]) + '_h.' + file_ext)
                issame = False
